...
"Daniel Wilson (2014)",
"",
"Usage: Rscript cfml_results_Arun_v2.R prefix [coresites_list]",
sep="\n")
...
#noncore.plot = noncore.len>=1000
#alt.image(plot.mut,col=c("skyblue","yellow","yellow"),xlab="Position",axes=FALSE,xaxs="i",yaxs="i",xpos=which(xref>0),background.fun=f)
...
#pdf(file="latest_cfml_phylogram.pdf",width=56,height=28)
pdf(file=paste0(prefix,".cfml_phylogramOnly.pdf"),width=56,height=28)
par(mfrow=c(1,2))
plot(tree,type="phylogram", cex = 0.2, edge.color = "blue")
dev.off()
...
####Figure out what the values in the plot are coming from
write.table(vpos, file = "vpos.txt")
write.table(lpos, file = "lpos.txt")
write.table(M_node_index, file = "Mnodeindex.txt")
write.table(M_anc_node, file = "Mancnode.txt")


pdf(file=paste0(prefix,".cfml_recomTree.pdf"),width=56,height=28)
par(mfrow=c(1,2))
...
###This is where I want to change colours
###Create a colour vector - how long does it have to be?
#no.colours <- length(lpos)
##Create a basic colour vector, just based on vertical position
#colourvector <- ifelse(vpos<100, "red", "blue")

##Create a colour vector trying to set particular nodeindexes to one colour
#I found the sequences I'm interested in in treelabels and figured out what position they are, in this case treelabels[90:99] I looked in M_node_index and found the positions that have those numbers

#positionsofinterest <- c(72,3,75,76,77,78,79,81,82,83)
#clusterA <- c(1,2,3)
#Create a colour vector of the right length that's all black
colourvector <- rep("blue", length(M_node_index))
#colourvector[positionsofinterest]<-"black"


#arrows(lpos,vpos,rpos,vpos,col=rainbow(length(lpos)),len=0)
arrows(lpos,vpos,rpos,vpos,col=colourvector,len=0)
...
sapply(sort(union(M_anc_node_index,c())),function(i) {
	...
	if(length(vpos)==2)	arrows(hpos,vpos[1],hpos,vpos[2],len=0,col=colourvector)
})
...
alt.image(spectrum.mut[,od],col=c("white","blue","yellow",colorRampPalette(c("orange","red"))(pmax(0,max(spectrum.mut)-2))),xlab="Position", cex=6, ylab = " ", axes=FALSE,xaxs="i",yaxs="i",xpos=which(xref>0),background.fun=background.noncore)
#axis(1);
axis(1, tck=1, at = NULL, col="grey", lty="dashed", cex.lab=3, cex.axis=3, cex=3);#(1, tck=1, at = set3, col="grey", lty="dashed", cex.lab=4, cex.axis=4, cex=4);
#axis(2, col="black", at=1:13, labels=FALSE)
axis(3, col="black", at=NULL, cex.lab=3, cex.axis=3, cex=3)#(3, col="black", at=set3, cex.lab=4, cex.axis=4, cex=4)
#axis(4, col="black", at=1:13, labels=FALSE)
print("info")
print(length(od))
print("spectrum.mut")
str(spectrum.mut)
...
print("treelabels")
str(treelabels)
print("M_anc_node_index")
str(M_anc_node_index)
write.csv(spectrum.mut, file = "spectrum.csv") # write out

#To remove the names I've commented out the axis command below
# axis(2,1:nrow(wh.mut),ifelse((1:nrow(wh.mut))<=ceiling(nrow(wh.mut)/2),rownames(wh.mut),"")[od],las=2,cex.axis=.4); box()
# Plot the recombination intervals
if(!is.na(istatefile)) {
